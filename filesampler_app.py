...
def filesampler():
    ...
    if total_files == 0:
        messagebox.showwarning("Warning",
                               "No files with '{}' extension were found in the '{}' folder.".format(file_ext, file_folder))
        return None
    ...
    if output_folder == "":
        messagebox.showwarning("Warning", "Output folder name cannot be an empty string. Using 'output' as output folder name.")
    ...
    messagebox.showinfo("Done", f"Files sampling complete!\n\nList of randomly sampled files copied to:\n\n{outputpath}\n\n"
